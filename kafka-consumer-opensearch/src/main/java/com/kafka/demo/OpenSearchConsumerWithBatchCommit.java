...
public class OpenSearchConsumerWithBatchCommit {
    public static final Logger log = LoggerFactory.getLogger(OpenSearchConsumerWithBatchCommit.class.getSimpleName());
        ...
        try(openSearchClient; consumer) {
            ...
            while(true){
                ...
                if(bulkRequest.numberOfActions() > 0){
                    //instead of normal index method
                    BulkResponse bulkResponse = openSearchClient.bulk(bulkRequest, RequestOptions.DEFAULT);

                    log.info("consumer offsets has been committed - {} record{s}", bulkResponse.getItems().length);
                    consumer.commitSync();
                }
